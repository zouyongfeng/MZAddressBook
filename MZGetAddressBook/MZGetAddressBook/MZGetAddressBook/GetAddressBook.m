...
+ (void)getOriginalAddressBook:(AddressBookArrayBlock)addressBookArray authorizationFailure:(AuthorizationFailure)failure
{
    ...
    dispatch_async(queue, ^{
        ...
        [kAddressBookHandle getAddressBookDataSource:^(PersonModel *model) {
...
+ (void)getOrderAddressBook:(AddressBookDictBlock)addressBookInfo authorizationFailure:(AuthorizationFailure)failure
{
    ...
    dispatch_async(queue, ^{
        ...
        [kAddressBookHandle getAddressBookDataSource:^(PersonModel *model) {
            ...
            {
                if (model.mobile.length > 0 && model.name.length > 0) {
...
+ (NSString *)getFirstLetterFromString:(PersonModel *)model
{
    NSMutableString *mutableString = [NSMutableString stringWithString:model.name];
    CFStringTransform((CFMutableStringRef)mutableString, NULL, kCFStringTransformToLatin, false);
    NSString *pinyinString = [mutableString stringByFoldingWithOptions:NSDiacriticInsensitiveSearch locale:[NSLocale currentLocale]];
    // 将拼音首字母装换成大写
    NSString *strPinYin = [[self polyphoneStringHandle:model.name pinyinString:pinyinString] uppercaseString];
    model.pinyingArray = [[strPinYin lowercaseString] componentsSeparatedByString:@" "];
    model.pinyingString = [[strPinYin stringByReplacingOccurrencesOfString:@" " withString:@""] lowercaseString];
    // 截取大写首字母
